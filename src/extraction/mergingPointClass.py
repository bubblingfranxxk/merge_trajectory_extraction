...
class MergingPointClass(object):
    ...
    def initTracks(self, tracks, row):
        ...
        try:
            ...
            # 没看懂为啥要限定trackId
            self.trackselftail = tracks[
                (tracks["frame"] <= (row["frame"] + 5)) & (tracks["trackId"] <= (row["trackId"]))].tail(10)
            self.tailLeadVehicles = np.unique(np.concatenate(
                (self.trackselftail["leadId"].values, self.trackselftail["leftLeadId"].values,
                 self.trackselftail["rightLeadId"].values),
                axis=0))
            self.tailRearVehicles = np.unique(np.concatenate(
                (self.trackselftail["rearId"].values, self.trackselftail["leftRearId"].values,
                 self.trackselftail["rightRearId"].values),
                axis=0))
    ...
    def run(self):
        ...
        for record in range(0, 93):
            ...
            for curid, curgroup in tracks[self.usedColumns].groupby("trackId"):
                ...
                ] = tmpconcat.apply(lambda row: self.matchSurroundingVehicles(row, tracksmeta), axis=1,
                                    result_type="expand")
    ...
    def calculateSafetyIndicatros(self, row1, row2):
        ...
        if leadData.empty:
            ...
            # distanceLead_这个变量有点问题
            distanceLead_ = np.dot((positionSelfVector - positionLead).T, (speedSelfVector - speedLead)) / distanceLead
            # print("array1: ", (positionSelfVector - positionLead))
            # print("array2: ", (speedSelfVector - speedLead))
            # print("distanceLead: ", distanceLead)
            # print("distanceLead_: ", distanceLead_)
            # 这个ttc好像是错的
    ...
    def getLateralSpeedandAcceleration(self, row):
        ...
        try:
            ...
            # 因为构造多项式的时候x为1，2，3，4，5……所以求出的值要比真实的值差25倍
            # 这个思路很省力
    ...
    def readCSVFile(self, curTracksId):
        ...
        PathTracksMeta = self.rootPath + r"\drone-dataset-tools-master\data\{}_tracksMeta.csv".format(str(curTracksId))
        PathRecordingMeta = self.rootPath + r"\drone-dataset-tools-master\data\{}_recordingMeta.csv".format(
            str(curTracksId))
        PathTracks = self.rootPath + r"\drone-dataset-tools-master\data\{}_tracks.csv".format(str(curTracksId))
