...
class TimeSeriesDataset(Dataset):
    def __init__(self, folder_path_ego, folder_path_lead, folder_path_rear, seq_len, output_dim, target_columns):
        self.data, self.extra_info, self.mask = load_csv_files_to_tensor(
            folder_path_ego,
            folder_path_lead,
            folder_path_rear,
            seq_len,
    ...
    def __getitem__(self, idx):
        return self.data[idx], self.extra_info[idx], self.mask[idx]
...
class TransformerGenerator(nn.Module):
    def __init__(self, input_dim, seq_len, d_model, num_heads, num_layers, output_dim, dropout_prob=0.1):
        ...
        self.fc_in = nn.Linear(input_dim, d_model)  # 注意修改这里的输入维度
...
# GAN 模型：生成器和判别器
class GAN:
    def __init__(self, generator, discriminator, data_columns, feature_columns, result_columns,gen_lr=5e-3, disc_lr=5e-4):
        ...
        self.scheduler_G = optim.lr_scheduler.StepLR(self.optim_G, step_size=200, gamma=0.5)
    ...
    def save_fake_data_to_csv(self,
        ...
        logger.info(f"Saved filtered fake data to {fake_data_file}.")
    ...
    def train(self, dataloader, noise_dim, epochs=1000):
        ...
        for epoch in range(epochs):
            ...
            for real_data, extra_info, mask in dataloader:
                ...
                fake_data = self.generator(noise, mask)
                ...
                # 2) 判别器 loss
                # 真实样本的判别损失
                real_pred = self.discriminator(real_data, mask)
                real_loss = self.criterion(real_pred, torch.ones_like(real_pred))

                # 生成样本的判别损失
                fake_pred = self.discriminator(fake_data.detach(), mask)
                ...
                # 生成样本的判别损失（欺骗判别器）
                fake_pred_for_g  = self.discriminator(fake_data, mask)
            ...
            if epoch % self.interval == 0:
                ...
                # 保存生成数据到 CSV
                self.save_fake_data_to_csv(all_fake_data, all_extra_info, all_masks, epoch, self.data_columns,
...
def load_csv_files_to_tensor(folder_path_ego, folder_path_lead, folder_path_rear, seq_len, output_dim, target_columns):
    ...
    return real_data, extra_info, mask
...
if __name__ == '__main__':
    ...
    discriminator = Discriminator(output_dim, seq_len, hidden_dim, num_layers)

    # 初始化 GAN
    logger.info(f"Initialize CGAN model.")
    cgan = GAN(generator, discriminator, target_columns, features, result_columns)
